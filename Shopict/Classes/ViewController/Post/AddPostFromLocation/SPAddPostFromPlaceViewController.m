...
@interface SPAddPostFromPlaceViewController ()

@end

@implementation SPAddPostFromPlaceViewController
...
- (void)viewDidLoad
{
    ...
    [self.navigationItem setTitle:@"FROM PLACE"];
    ...
    [self.images addObject:self.capturedImage];
    ...
    self.currencyTextField.text = code;
}
...
- (void)sendAddProductRequest
{
    ...
    [self sendAddPostFromPlaceRequestWithPostType:SHARE name:self.titleTextField.text price:[price floatValue] currency:currencyCode description:nil categoryIds:categories tag:nil img0:image0 img1:image1 img2:image2 img3:image3 img4:image4 img5:image5 img6:image6 img7:image7 comment:comment delegate:self foursquareId:self.selectedVenue.fourSquareId placeName:self.selectedVenue.name address:self.selectedVenue.address countryCode:self.selectedVenue.countryCode latitude:self.selectedVenue.latitude longitude:self.selectedVenue.longitude];
}
...
- (void)SPAddPostFromPlaceRequestDidFinish:(SPGetPostInfoResponseData*)response
{
    ...
    if (response.error) {
        ...
        if ([self.delegate respondsToSelector:@selector(SPAddPostFromPlaceViewControllerDidUploadNewPost:)]) {
            [self.delegate SPAddPostFromPlaceViewControllerDidUploadNewPost:response.post];
        }
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField
{
    ...
    if ([textField isEqual:self.placeTextField]) {
        ...
        [self goToLocationSelectionControllerWithDelegate:self];
        return NO;
    }
...
//        if (![self.placeTextField.text isValid]) {
...
//    
    if (![self.placeTextField.text isValid]) {
        [self showErrorAlert:@"Please select the place"];
        return NO;
    }
...
- (IBAction)importImageFromWebButtonPressed:(id)sender
{
    ...
    webImportViewController.websiteURL = self.placeTextField.text;
...
{
    self.selectedVenue = location;
    self.placeTextField.text = self.selectedVenue.name;
}
